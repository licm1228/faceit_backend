...
import { ArrowLeft, CheckCircle2, FileText, Target } from "lucide-react";
...
export function InterviewReportPage() {
  ...
  return (
    <MainLayout>
      <div className="flex h-[calc(100vh-4rem)] flex-col bg-[#FAFAFA]">
                ...
                <>
                  ...
                  <div className="rounded-3xl border border-[#E5E7EB] bg-white p-6">
                    <div className="mb-4 flex items-center gap-2 text-[#0F172A]">
                      ...
                      <p className="font-semibold">练习路径</p>
                    </div>
                    <div className="space-y-4">
                      <div className="rounded-2xl bg-[#F8FAFC] px-4 py-4 text-sm leading-6 text-[#334155]">
                        {report.practiceSummary || "建议先补核心概念，再做同类题专项复练，最后做一次完整模拟复盘。"}
                      </div>
                      {report.practicePlan?.length ? (
                        <div className="grid gap-4 lg:grid-cols-3">
                          {report.practicePlan.map((plan) => (
                            <div key={`${plan.day}-${plan.title}`} className="rounded-2xl border border-[#E2E8F0] bg-white p-4">
                              <div className="flex items-center justify-between">
                                <span className="rounded-full bg-[#DBEAFE] px-2.5 py-1 text-[11px] font-medium text-[#1D4ED8]">
                                  Day {plan.day}
                                </span>
                                {plan.focus ? <span className="text-xs text-[#64748B]">{plan.focus}</span> : null}
                              </div>
                              <p className="mt-3 text-sm font-semibold text-[#0F172A]">{plan.title}</p>
                              {plan.actions?.length ? (
                                <div className="mt-3 space-y-2">
                                  {plan.actions.map((action, index) => (
                                    <p key={`${plan.title}-${index}`} className="rounded-xl bg-[#F8FAFC] px-3 py-2 text-xs leading-5 text-[#334155]">
                                      {action}
                                    </p>
                                  ))}
                                </div>
                              ...
                              {plan.expectedOutcome ? (
                                <p className="mt-3 text-xs leading-5 text-[#64748B]">目标：{plan.expectedOutcome}</p>
                              ) : null}
                            </div>
                          ))}
                        </div>
                    ...
                    <div className="space-y-3">
                      {report.recommendedPractices.length === 0 ? (
                        ...
                        report.recommendedPractices.map((practice) => (
                          <div key={practice.id} className="rounded-2xl border border-[#E2E8F0] bg-[#F8FAFC] px-4 py-4">
                            <div className="flex flex-wrap items-center gap-2">
                              {practice.priorityLabel ? (
                                <span className="rounded-full bg-[#DBEAFE] px-2.5 py-1 text-[11px] font-medium text-[#1D4ED8]">
                                  {practice.priorityLabel}
                                </span>
                              ) : null}
                              ...
                              {practice.estimatedMinutes ? (
                                <span className="text-xs text-[#64748B]">建议 {practice.estimatedMinutes} 分钟</span>
                              ) : null}
                            </div>
                            <p className="mt-3 text-sm font-medium text-[#0F172A]">{practice.questionText}</p>
                            {practice.focusArea || practice.skillGap ? (
                              <div className="mt-3 grid gap-3 md:grid-cols-2">
                                {practice.focusArea ? (
                                  <div className="rounded-xl bg-white px-3 py-3 text-xs leading-5 text-[#334155]">
                                    <p className="font-medium text-[#0F172A]">重点补哪一块</p>
                                    <p className="mt-1">{practice.focusArea}</p>
                                  </div>
                                ) : null}
                                {practice.skillGap ? (
                                  <div className="rounded-xl bg-white px-3 py-3 text-xs leading-5 text-[#334155]">
                                    <p className="font-medium text-[#0F172A]">当前短板</p>
                                    <p className="mt-1">{practice.skillGap}</p>
                                  </div>
                                ) : null}
                              </div>
                            ) : null}
                            {practice.recommendationReason ? (
                              <p className="mt-3 text-xs leading-5 text-[#334155]">{practice.recommendationReason}</p>
                            ) : null}
                            ...
                            {practice.practiceMethod?.length ? (
                              <div className="mt-3">
                                <p className="text-xs font-medium text-[#0F172A]">怎么练</p>
                                <div className="mt-2 space-y-2">
                                  {practice.practiceMethod.map((step, index) => (
                                    <p key={`${practice.id}-method-${index}`} className="rounded-xl bg-white px-3 py-2 text-xs leading-5 text-[#334155]">
                                      {step}
                                    </p>
                                  ))}
                                </div>
                            ...
                            {practice.answerChecklist?.length ? (
                              <div className="mt-3">
                                <p className="text-xs font-medium text-[#0F172A]">答题检查点</p>
                                <div className="mt-2 flex flex-wrap gap-2">
                                  {practice.answerChecklist.map((item) => (
                                    <span key={`${practice.id}-check-${item}`} className="rounded-full border border-[#D6E4FF] bg-white px-2.5 py-1 text-[11px] text-[#475467]">
                                      {item}
                                    </span>
                                  ))}
                                </div>
